...
# Load CSV data for Hospital 1 (Replace with real dataset)
data = pd.read_csv("data/hospital_1_patients.csv")  # ✅ Use the clean dataset for training
...
torch.save(model.state_dict(), "models/hospital_1_weights.pth")  # Save weights
print("✅ Weights saved: models/hospital_1_weights.pth")

# Upload weights to aggregator (for federated learning)
url = "http://127.0.0.1:5000/upload"  # Aggregator URL
files = {'file': open("models/hospital_1_weights.pth", 'rb')}  # Prepare file for upload
...
sample = torch.tensor([[55, 180, 125, 34.0, 1]], dtype=torch.float32)  # Sample input
with torch.no_grad():
