...
# Load CSV data for Hospital 3 (Replace with real dataset)
data = pd.read_csv("data/hospital_3_patients.csv")  # ✅ Use the clean dataset for training
...
torch.save(model.state_dict(), "models/hospital_3_weights.pth")  # Save weights
print("✅ Weights saved: models/hospital_3_weights.pth")

# Upload weights to aggregator (for federated learning)
url = "http://127.0.0.1:5000/upload"  # Aggregator URL
files = {'file': open("models/hospital_3_weights.pth", 'rb')}  # Prepare file for upload
...
sample = torch.tensor([[55, 200, 90, 23.0, 0]], dtype=torch.float32)  # Sample input
with torch.no_grad():
